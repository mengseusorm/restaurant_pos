...
class BranchController extends AdminController
{
    ...
    public function index(PaginateRequest $request
        ...
        try {
            ...
            if ($request->has('last_updated') && $branches->isEmpty()) {
                return response([
                    'status' => true,
    ...
    public function show(\Illuminate\Http\Request $request, Branch $branch
        ...
        try {
            $branchData = $this->branchService->show($branch);
            
            // Check if last_updated parameter was provided
            if ($request->has('last_updated') && $branchData && isset($branchData->updated_at)) {
                if ($branchData->updated_at->toIso8601String() <= $request->get('last_updated')) {
                    return response([
                        'status' => true,
                        'message' => 'No branch updated',
                        'has_updates' => false
                    ], 200);
                }
            }
            
            return new BranchResource($branchData);
        } catch (Exception $exception) {
            return response(['status' => false, 'message' => $exception->getMessage()], 422);
        }
    }

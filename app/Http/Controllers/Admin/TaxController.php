...
class TaxController extends AdminController
{
    ...
    public function destroyFromItem(Tax $tax): \Illuminate\Http\Response|\Illuminate\Contracts\Foundation\Application|\Illuminate\Contracts\Routing\ResponseFactory
    {
        try {
            $this->taxService->destroyFromItem($tax);
            return response('', 202);
        } catch (Exception $exception) {
            return response(['status' => false, 'message' => $exception->getMessage()], 422);
        }
    ...
    public function applyTaxToAllItem(Tax $tax): \Illuminate\Http\Response|\Illuminate\Contracts\Foundation\Application|\Illuminate\Contracts\Routing\ResponseFactory
    {
        try {
            $this->taxService->applyTaxToAllItem($tax);
            return response(['status' => true, 'message' => 'Tax applied to all items successfully.'], 200);
        } catch (Exception $exception) {
            return response(['status' => false, 'message' => $exception->getMessage()], 422);
        }
    }

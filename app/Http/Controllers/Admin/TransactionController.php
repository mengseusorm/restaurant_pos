...
class TransactionController extends AdminController
{
    ...
    public function voidTransaction(\Illuminate\Http\Request $request)
    {
        try {
            $transaction = $this->orderService->voidTransaction($request->transaction_id, $request->void_amount);
            return response(['status' => true, 'message' => 'Transaction voided successfully', 'data' => new TransactionResource($transaction)], 200);
        } catch (Exception $exception) {
            return response(['status' => false, 'message' => $exception->getMessage()], 422);
        }
    }

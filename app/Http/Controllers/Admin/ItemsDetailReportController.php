...
class ItemsDetailReportController extends AdminController
{
    ...
    public function __construct(ItemService $itemService,CompanyService $companyService, ThemeService $themeService)
    {
        ...
        $this->middleware(['permission:items-detail-report'])->only('index', 'export', 'exportDetail', 'pdf');
    }

    public function index(PaginateRequest $request) : \Illuminate\Http\Response | \Illuminate\Http\Resources\Json\AnonymousResourceCollection | \Illuminate\Contracts\Foundation\Application | \Illuminate\Contracts\Routing\ResponseFactory
    {
        try { 
            return ItemDetailReportResource::collection($this->itemService->itemReportDetailList($request)); 

    ...
    public function list(PaginateRequest $request)
    {
        try {
            ...
            return Transaction::with('order')->where(function ($query) use ($requests) {
                ...
                if(isset($requests['payment_method'])) { 
                    $paymentMethodId = (int)$requests['payment_method']; 
                    $query->where(function($subQuery) use ($paymentMethodId) {
                        $subQuery->where('payment_method', $paymentMethodId)
                                 ->orWhere('pos_payment_method', $paymentMethodId);
                    });
                } 

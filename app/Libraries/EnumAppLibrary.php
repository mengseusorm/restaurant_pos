...
class EnumAppLibrary {
    ...
    public static function yesNoStatus($status): int
    {
        $status = strtolower(trim($status));
        if ($status === 'yes' || $status === 'y' || $status === '1' || $status === 'true' || $status === 'active') {
            return Ask::YES;
        } elseif ($status === 'no' || $status === 'n' || $status === '0' || $status === 'false' || $status === 'inactive') {
            return Ask::NO;
        }
        return Ask::NO;
    }

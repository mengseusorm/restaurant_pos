    ...
    actions: {
        ...
        save(context, payload) {
            return new Promise((resolve, reject) => {
                ...
                let url    = '/admin/room';
                if (this.state['room'].temp.isEditing) {
                    method = axios.post;
                    url    = `/admin/room/${this.state['room'].temp.temp_id}`;
                }
        ...
        destroy(context, payload) {
            return new Promise((resolve, reject) => {
                axios.delete(`admin/room/${payload.id}`).then((res) => {
                    context.dispatch('lists', payload.search).then().catch();
                    resolve(res);
                }).catch((err) => { reject(err); });
            });
        ...
        show(context, payload) {
            return new Promise((resolve, reject) => {
                axios.get(`admin/room/show/${payload}`).then((res) => {
        ...
        changeStatus(context, payload) {
            return new Promise((resolve, reject) => {
                axios.post(`admin/room/${payload.id}/change-status`, { status: payload.status }).then((res) => {
                    context.dispatch('lists', payload.search).then().catch();
                    resolve(res);
                }).catch((err) => { reject(err); });
            });

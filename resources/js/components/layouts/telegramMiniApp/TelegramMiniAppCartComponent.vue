<template>
    <aside id="telegram-mini-app-cart"
...
export default {
    name: "TelegramMiniAppCartComponent",
    computed: {
        setting: function () {
    ...
    methods: {
        ...
        closeSidebar: function () {
            const cart = document.getElementById('telegram-mini-app-cart');
        ...
        quantityUp: function (id, e) {
            if (e.target.value > 0) {
                this.$store.dispatch('telegramMiniApp/cart/quantity', { id: id, status: e.target.value }).then().catch();
            }
        },
        quantityIncrement: function (id) {
            this.$store.dispatch('telegramMiniApp/cart/quantity', { id: id, status: "increment" }).then().catch();
        },
        quantityDecrement: function (id) {
            this.$store.dispatch('telegramMiniApp/cart/quantity', { id: id, status: "decrement" }).then().catch();
        }
    },
}
</script>
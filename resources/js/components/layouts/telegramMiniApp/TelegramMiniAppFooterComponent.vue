            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-5 py-8">
                ...
                <nav v-if="pages.length > 0"  class="flex items-center gap-6">
                    <router-link v-for="page in pages" class="text-sm capitalize text-white"
                        :to="{ name: 'telegram.mini.app.page', params: { slug: this.$route.params.slug , pageSlug: page.slug } }">
                        {{ page.title }}
                    </router-link>
                </nav>
            </div>
...
export default {
    name: "TelegramMiniAppFooterComponent",
    ...
    computed: {
        ...
        pages: function () {
            return this.$store.getters['telegramMiniApp/page/lists'];
        }
    },
    mounted() {
        this.loading.isActive = true;
        this.$store.dispatch("telegramMiniApp/page/lists", {
            paginate: 0,
            order_column: "id",
            order_type: "asc",

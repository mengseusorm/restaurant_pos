        ...
        <div class="max-w-sm w-full h-screen absolute top-0 right-0 translate-x-full bg-white transition">
            ...
            <div v-if="carts.length > 0" class="p-4">
                ...
                <router-link @click.prevent="closeSidebar"
                    :to="{ name: 'online.order.checkout', params: { slug: this.$route.params.slug } }"
...
export default {
    ...
    methods: {
        ...
        closeSidebar: function () {
            const cart = document.getElementById('online-order-cart');
        ...
        quantityUp: function (id, e) {
            if (e.target.value > 0) {
                this.$store.dispatch('onlineOrderCart/quantity', { id: id, status: e.target.value }).then().catch();
            }
        },
        quantityIncrement: function (id) {
            this.$store.dispatch('onlineOrderCart/quantity', { id: id, status: "increment" }).then().catch();
        },
        quantityDecrement: function (id) {
            this.$store.dispatch('onlineOrderCart/quantity', { id: id, status: "decrement" }).then().catch();
        }
    }
}
</script>
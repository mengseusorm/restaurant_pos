    ...
    <header class="shadow-xs bg-white ff-header"  ref="ffHeader">
        <div class="container flex flex-col lg:flex-row items-center justify-between">
            <div class="w-full flex items-center justify-between gap-3 xl:gap-8 lg:justify-start lg:w-fit">
                <router-link :to="{ name: 'online.order.menu', params: { slug: this.$route.params.slug } }"
                    class="flex-shrink-0">
                ...
                <div class="flex items-center gap-2 lg:hidden">
                    ...
                    <button
                        class="online-order-webcart flex items-center justify-center gap-1.5 w-fit rounded-3xl capitalize text-sm font-medium h-8 px-2.5 transition text-white bg-heading">
            ...
            <div class="flex flex-col items-center justify-end gap-3 w-full mt-4 lg:flex-row lg:w-fit lg:mt-0">
                ...
                <button
                    class="online-order-webcart hidden lg:flex items-center justify-center gap-1.5 w-fit rounded-3xl capitalize text-sm font-medium h-8 px-3 transition text-white bg-heading">
                    <i class="fa-solid fa-bag-shopping text-sm"></i>
...
export default {
    name: "OnlineOrderNavbarComponent",
    ...
    mounted() {
        ...
        this.$store.dispatch('frontendSetting/lists').then(res => {
            ...
            window.setTimeout(() => {
                this.$store.dispatch('onlineOrderBranch/show', this.$route.params.slug).then(res => {
                    // this.$store.dispatch('onlineOrderCart/initTable', res.data.data);

                    this.$store.dispatch('frontendBranch/show', res.data.data.branch_id).then(res => {
                        // location.reload();
                    }).catch();
                }).catch((err) => { });
            }, 300);
    ...
    methods: {
        ...
        search: function () {
            if (typeof this.searchItem !== "undefined" && this.searchItem !== "") {
                this.$router.push({ name: "online.order.search", query: { s: this.searchItem } });
                this.searchItem = "";
            }
        },

...
export default {
    ...
    mounted() {
        // Initialize language code from $i18n locale
        const currentLocale = this.$i18n.locale;
        localStorage.setItem('language_code', currentLocale);
    },
    methods: {
        login: function () {
            try {
                ...
                this.$store.dispatch('login', this.form).then((res) => {
                    ...
                    if (Number(res.data.user?.role_id) === roleEnum.THERAPIST) {
                        router.push({name: "therapist.room.list"});
                    } else {

<template>
    ...
    <section class="pt-8 pb-16">
        <div class="container max-w-3xl">
            <div class="flex items-start flex-col md:flex-row gap-6">
                <div class="w-full">
                    <div class="p-4 mb-6 rounded-2xl shadow-xs bg-white">
                        ...
                        <div class="my-6 flex flex-col items-center justify-center gap-2 bg-gray-50 rounded-lg p-4 border border-gray-200">
                            ...
                            <button v-if="isMobile && !isWechatBrowser" @click="openHuioneApp" class="mt-2 px-5 py-2 rounded-md bg-primary text-white font-medium hover:bg-primary-dark transition-colors duration-150" type="button">Pay with Huione App</button>
                        </div>

                        <!-- <TelegramMiniAppOrderStatusComponent :props="order" /> -->
                ...
                <div class="w-full rounded-2xl shadow-xs bg-white">
                    ...
                    <!-- <div class="p-4">
                        <TelegramMiniAppOrderReceiptComponent :order="order" :orderBranch="orderBranch" :orderItems="orderItems" />
                    </div> -->
                </div>
            </div>
        </div>
        ...
        <div class="fixed bottom-4 left-0 right-0 z-50 px-4">
            <div class="flex items-center gap-3 bg-white/30 backdrop-blur-sm rounded-full shadow-lg border border-gray-200 px-4 py-2 w-full">
            <!-- Home Button -->
            <router-link 
                :to="{ name: 'telegram.mini.app.menu', params: { slug: this.$route.params.slug } }"
                class="flex items-center justify-center gap-2 px-4 py-2 rounded-full bg-primary/80 hover:bg-primary-dark/80 transition-colors duration-200 w-full">
                <i class="fa-solid fa-home text-sm text-white"></i>
                <span class="text-sm font-medium text-white whitespace-nowrap">{{ $t('label.home') || 'Home' }}</span>
            </router-link>
            </div>
        </div>
    </section>
...
import TelegramMiniAppOrderStatusComponent from '../../telegramMiniApp/components/TelegramMiniAppOrderStatusComponent.vue';
import TelegramMiniAppOrderReceiptComponent from '../../telegramMiniApp/order/TelegramMiniAppOrderReceiptComponent.vue';
...
export default {
    name: 'TelegramMiniAppMakePaymentComponent',
    components: { TelegramMiniAppOrderReceiptComponent, TelegramMiniAppOrderStatusComponent, TelegramMiniAppLoadingComponent },
    data() {
        return {
            loading: {
                isActive: false,
    ...
    mounted() {
        ...
        if (this.$route.params.id) {
            this.loading.isActive = true;
            this.$store
                .dispatch('telegramMiniApp/order/show', this.$route.params.id)
                .then((res) => {
                    ...
                    if (res.data.data.payment_status == this.enums.paymentStatusEnum.PAID) {
                        this.$router.push({ name: 'telegram.mini.app.order.details', params: { id: this.$route.params.id } });
                    }
                })
                .catch((error) => {
                    this.loading.isActive = false;
    ...
    methods: {
        placeOrder() {
            this.$store
                .dispatch('onlineOrderHuionePayment/placeOrder', this.$route.params.id)
                .then((res) => {
                    this.loading.isActive = false;
        ...
        checkPaymentStatus() {
            this.$store
                .dispatch('onlineOrderHuionePayment/paymentStatus', this.$route.params.id)
                .then((res) => {
                    ...
                    if (res.data.status === true || res.data.paymentStatus === 'DONE_PAYMENT') {
                        ...
                        alertService.successFlip(
                            1,
                            this.$t("message.payment_success"),
                        );
                        this.$router.push({ name: 'telegram.mini.app.order.details', params: { id: this.$route.params.id } });
                    }
                })
        ...
        openHuioneApp() {
            const huioneLink = `huione://huione/openReceive?code=${this.code}`;        // Use Telegram WebApp API to open external links
            if (window.Telegram && window.Telegram.WebApp) {
                try {
                    // For Telegram Mini Apps, we need to use a workaround for deep links
                    // Create a temporary link element and trigger click
                    const link = document.createElement('a');
                    link.href = huioneLink;
                    link.target = '_blank';
                    link.rel = 'noopener noreferrer';
                    
                    // Try Telegram's openLink for external opening - without unsupported parameters
                    // Note: try_instant_view is not supported in Telegram WebApp v6.0
                    window.Telegram.WebApp.openLink(huioneLink);
                    
                    // Also trigger the link click as fallback using requestAnimationFrame
                    ...
                    console.error('Error opening Huione app via Telegram:', error);
                    // Direct fallback using native methods
                    try {
                        const link = document.createElement('a');
                        link.href = huioneLink;
                        link.target = '_blank';
                        ...
                        alertService.error('Unable to open Huione app. Please try scanning the QR code instead.');
                    }
                }
            } else {
                // Fallback for non-Telegram environments (testing purposes)
                const ua = navigator.userAgent.toLowerCase();

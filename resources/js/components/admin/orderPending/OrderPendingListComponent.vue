    ...
    <div class="col-12">
        <div class="db-card">
            <div class="db-card-header border-none">
                <h3 class="db-card-title">{{ $t('menu.pending_orders') }}</h3>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 sm:col-6 md:col-4 xl:col-3">
                            ...
                            <vue-select class="db-field-control f-b-custom-select" id="searchStatus"
                                v-model="props.search.status" :options="orderStatus || []" :label-by="(option) => option?.['name_' + language_code] || option?.name || 'Unknown'" :value-by="'status_code' || null" :closeOnSelect="true" :searchable="true"
            ...
            <div class="db-table-responsive">
                <table class="db-table stripe" id="print" :dir="direction">
                    ...
                    <tbody class="db-table-body" v-if="orders.length > 0">
                        <tr class="db-table-body-tr cursor-pointer" v-for="(order, index) in orders" :key="order?.id || index"
                        @click="orderShow(order)">

                        <td class="db-table-body-td">{{ order?.order_datetime || 'N/A' }}</td>

                        <td class="db-table-body-td">
                            {{ order?.order_serial_no || 'N/A' }}
                        </td>
                        <td class="db-table-body-td">
                                #{{ order?.waiting_number || 'N/A' }} {{ order?.token ? '/ ' + order.token : '' }}
                            </td>
                            <td class="db-table-body-td">
                                <div v-if="order.order_dinings && order.order_dinings.length > 0">
                                    <span v-for="dining in order.order_dinings" :key="dining?.id || index" class="db-badge blue ml-2">
                                        <span>{{ dining?.dining_table_name || 'N/A' }}</span>
                                    </span>
                                </div>
                            </td>
                            ...
                            <td class="db-table-body-td">
                               <span class="db-table-badge text-green-600 bg-green-100">
                                    {{ order.order_type_info?.['name_' + language_code] || order.order_type_info?.name || 'N/A' }}
                                </span>
                            </td>
                            ...
                            <td class="db-table-body-td">{{ order?.total_currency_price || '0.00' }}</td>
                            <td class="db-table-body-td">{{ order?.total_tax_currency_price || '0.00' }}</td>
                            <td class="db-table-body-td">{{ order?.total_amount_price || '0.00' }}</td>
                            <td class="db-table-body-td">
                                <span :class="orderStatusClass(order.order_status_info?.name)">
                                    {{ order.order_status_info?.['name_' + language_code] || order.order_status_info?.name || 'N/A' }}
                                </span>
                            </td>
                            <td class="db-table-body-td">
                                {{ enums.sourceEnumArray[order?.source] || 'N/A' }}
                            </td>
                            <td class="db-table-body-td">
                                <span :class="'text-xs capitalize h-5 leading-5 px-2 rounded-3xl text-[#FB4E4E] bg-[#FFDADA]' + statusClass(order?.payment_status)">
                                {{ enums.paymentStatusEnumArray[order?.payment_status] || 'N/A' }}
                            </span>
                            </td>
                            <td class="db-table-body-td hidden-print" v-if="permissionChecker('pos-orders')">
                                <div class="flex justify-start items-center sm:items-start sm:justify-start gap-1.5">
                                    <SmIconViewComponent :link="'admin.pos.orders.show'" :id="order?.id" v-if="permissionChecker('pos-orders')" />
                                </div>
                            </td>
                        </tr>
                    </tbody>
...
export default {
    name: "OrderPendingListComponent",
    components: {
        TableLimitComponent,
        PaginationSMBox,
        PaginationBox,
    ...
    computed: {
        ...
        orders: function () {
            return this.$store.getters['orderPending/lists'] || [];
        },
        customers: function () {
            return this.$store.getters['user/lists'] || [];
        },
        pagination: function () {
            return this.$store.getters['orderPending/pagination'] || {};
        },
        paginationPage: function () {
            return this.$store.getters['orderPending/page'] || 1;
        },
        direction: function () {
        ...
        language_code: function () {
            return this.$store.getters['frontendLanguage/show']?.language_code || 'en';
        },
    },
    methods: {
        orderShow(order) {
            if (order) {
                if (order.source == this.enums.sourceEnum.ONLINE_ORDER) {
                    this.$router.push({ name: 'admin.online.order.show', params: { id: order.id } });
                } else if (order.source == this.enums.sourceEnum.TELEGRAM_MINI_APP) {
                    this.$router.push({ name: 'admin.telegram.mini.app.order.show', params: { id: order.id } });
                } else if (order.source == this.enums.sourceEnum.TABLE) {
        ...
        list: function (page = 1) {
            ...
            this.$store.dispatch('orderPending/lists', this.props.search).then(res => {
        ...
        destroy: function (id) {
            ...
            appService.destroyConfirmation().then((res) => {
                try {
                    this.loading.isActive = true;
                    this.$store.dispatch('orderPending/destroy', { id: id, search: this.props.search }).then((res) => {
                        this.loading.isActive = false;

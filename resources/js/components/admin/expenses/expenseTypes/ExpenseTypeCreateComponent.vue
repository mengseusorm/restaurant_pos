<template>
    ...
    <div id="sidebar" class="drawer">
        <div class="drawer-header">
            <h3 class="drawer-title">{{ $t('menu.expense_types') }}</h3>
...
export default {
    name: "ExpenseTypeCreateComponent",
    components: {
    ...
    data() {
        return {
            ...
            addButton: {
                title: this.$t("button.add_expense_type")
            },
            errors: {}
        }
    },
    ...
    methods: {
        ...
        reset: function () {
            ...
            this.$store.dispatch('expenseType/reset').then().catch();
            this.errors = {};
        ...
        save: function () {
            try {
                ...
                const tempId = this.$store.getters['expenseType/temp'].temp_id;

                this.$store.dispatch('expenseType/save', this.props).then(res => {
                    appService.sideDrawerHide();
                    this.loading.isActive = false;
                    alertService.successFlip((tempId === null ? 0 : 1), this.$t('menu.expense_types'));
                    this.reset();
                }).catch((err) => {
                    this.loading.isActive = false;

<template>
    ...
    <SmModalCreateComponent :props="addAdustStockButton" />

    <div id="adjust-stock-record-modal" class="modal">
        <div class="modal-dialog">
            <div class="modal-header">
                <h3 class="modal-title">{{ $t('label.adjust_stock_record') }}</h3>
                <button class="modal-close fa-solid fa-xmark text-xl text-slate-400 hover:text-red-500"
                    @click="reset"></button>
            </div>
            <div class="modal-body">
                <form id="adjust-stock-record-form" @submit.prevent="saveAdjustStock">
                    <div class="form-row">
                        <div class="form-col-12">
                            <label for="item_category_id" class="db-field-title required">{{ $t("label.warehouse")
                                }}</label>
                            <vue-select class="db-field-control f-b-custom-select" id="item_category_id"
                                v-bind:class="errors.stock_id ? 'invalid' : ''" v-model="props.form.stock_id"
                                :options="warehouses" label-by="name" value-by="id" :closeOnSelect="true"
                        ...
                        <!-- <div class="form-col-12 sm:form-col-6">
                            <label for="item_category_id" class="db-field-title required">{{
                                $t("label.stock_record_type")
                                }}</label>
                            <span class="db-field-control">{{ $t("label.stock_adjustment") }}</span>
                        </div> -->
                        <div class="form-col-12">
...
export default {
    name: "StockRecordCreateAdjustStockComponent",
    ...
    computed: {
        addAdustStockButton: function () {
            return { title: this.$t('button.adjust_stock'), modalId: '#adjust-stock-record-modal' };
        },
    ...
    methods: {
        ...
        saveAdjustStock: function () {
            console.log("Save Adjust Stock");
            try {
                ...
                fd.append('record_type', 'ADJUST_STOCK');
                fd.append('quantity',this.props.form.quantity)
                 
                // const data = {
                //     item_id:this.props.form.item_id,
                //     stock_id:this.props.form.stock_id,
                //     user_id:this.props.form.user_id,
                //     order_id:0,
                //     record_type: 'ADJUST_STOCK',
                //     quantity:this.props.form.quantity
                // }
                
                const tempId = this.$store.getters['stockRecord/temp'].temp_id;
                this.loading.isActive = true;
                this.$store.dispatch('stockRecord/save', {
                    form: fd,

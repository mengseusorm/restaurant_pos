                ...
                <tr class="db-table-body-tr" v-for="tax in taxes" :key="tax">
                    ...
                    <td class="db-table-body-td">
                        <div class="flex justify-start items-center sm:items-start sm:justify-start gap-1.5">
                            ...
                            <button @click="applyTaxToAllItem(tax.id)" class="db-btn-outline sm success modal-btn m-0.5">
                                <span>{{ $t("button.apply") }} {{ $t("label.item") }}</span>
                            </button>
                            <SmDeleteComponent @click="destroy(tax.id)"/>
                            <button @click="destroyTaxFromItem(tax.id)" class="db-btn-outline sm danger modal-btn m-0.5">
...
export default {
    ...
    methods: {
        ...
        destroyTaxFromItem: function (id) {
            appService.destroyConfirmation().then((res) => {
                try {
                    this.loading.isActive = true;
                    this.$store.dispatch("tax/destroyTaxFromItem", {
                        id: id,
                        search: this.props.search,
                    }).then((res) => {
                        this.loading.isActive = false;
        ...
        applyTaxToAllItem: function (id) {
            VueSimpleAlert.confirm(
                this.$t("message.tax_apply_to_all_item_content"),
                this.$t("message.tax_apply_to_all_item") +"?",
                "warning",
                    {
                ...
                if (confirmed) {
                    this.loading.isActive = true;
                    this.$store.dispatch("tax/applyTaxToAllItem", {
                        id: id,
                        search: this.props.search,
                    }).then(() => {
                        this.loading.isActive = false;

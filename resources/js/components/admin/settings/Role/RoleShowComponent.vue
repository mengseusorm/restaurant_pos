                        ...
                        <tbody v-if="permissions.length > 0" class="db-table-body">
                            <tr v-for="permission in permissions" :key="permission" class="db-table-body-tr">
                                <td class="db-table-body-td">
                                    <div class="custom-checkbox">
                                        <input type="checkbox" class="custom-checkbox-field"
                                            :name="'feature_' + permission.id"
                                            :value="permission.id"
                                            :id="'feature_' + permission.id"
                                            v-model="checkedStatue[permission.id]"
                                ...
                                <td v-if="permission.children" v-for="children in permission.children" :key="children"
                                    ...
                                    <div class="custom-checkbox">
                                        <input type="checkbox" class="custom-checkbox-field"
                                            ...
                                            :value="children.id"
                                            v-model="checkedStatue[children.id]"
                                            :disabled="!disabledStatue[children.id]"
                                            @change="enable(children, 1, $event)">
                                        <i class="fa-solid fa-check custom-checkbox-icon"></i>
                                    </div>
                                </td>
    ...
    methods: {
        checkAll(event) {
            ...
            this.permissions.forEach(permission => {
                this.checkedStatue[permission.id] = this.allChecked;
                if (permission.children) {
                    permission.children.forEach(child => {
                        this.checkedStatue[child.id] = this.allChecked;
                        this.disabledStatue[child.id] = this.allChecked;
                    });
                }
            });
            ...
            if (this.allChecked) {
                this.permissions.forEach(permission => {
                    this.form.push(permission.id);
                    if (permission.children) {
                        permission.children.forEach(child => {
                            this.form.push(child.id);
                        });
                    }
                });
            }

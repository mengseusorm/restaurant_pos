    ...
    <div id="payment" class="db-tab-div active">
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 mb-5">
            <button @click="selectActive(index)"
                    ...
                    :key="paymentGateway.id || paymentGateway.slug || index" :class="index === selectIndex ? 'active' : ''">
            ...
            <div v-if="paymentGateways.length > 3" class="dropdown-group w-full">
                ...
                <div class="w-full dropdown-list absolute top-[42px] right-0 z-30 p-2 rounded-md bg-white shadow-lg">
                    <button @click="selectActive(index+3)"
                            ...
                            :key="paymentGateway.id || paymentGateway.slug || index" :class="index+3 === selectIndex ? 'active' : ''">
                        {{ paymentGateway.name }}
                    </button>
                </div>
            </div>
                ...
                <form @submit.prevent="save(index)" :id="'formElem' + index">
                    <div class="form-row">
                        ...
                        <div class="form-col-12">
                            ...
                            <button type="button" v-if="paymentGateway.slug === 'abapayway'" class="db-btn text-white bg-green-600 ms-4" @click="$router.push({ name: 'admin.settings.connectABAPayment' })">
                                <i class="lab lab-payment-gateway text-sm"></i>
                                {{ $t("menu.connect_aba_payment") }}
                            </button>
                        </div>
                    </div>
                </form>
    ...
    data() {
        return {
            ...
            search: {
                ...
                order_column: "id",
                order_type: "asc",
                // Remove the default excepts filter so the list shows the only available gateway when there is just one.
                // excepts: "1|2"
            },
            selectIndex: 0,
            enums: {

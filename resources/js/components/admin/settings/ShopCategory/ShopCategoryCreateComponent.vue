    ...
    <div id="categoryModal" class="modal">
        <div class="modal-dialog">
            <div class="modal-header">
                <h3 class="modal-title">{{ $t('menu.shop_categories') }}</h3>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-col-12">
                            ...
                            <input v-model="props.form.sort" v-bind:class="errors.sort ? 'invalid' : ''" type="text"
                                id="sort" class="db-field-control">
...
export default {
    name: "ShopCategoryCreateComponent",
    ...
    computed: {
        addButton: function () {
            return { title: this.$t('button.shop_category') };
        },
    },
    methods: {
        reset: function () {
            appService.modalHide();
            this.$store.dispatch('shopCategory/reset').then().catch();
            this.errors = {};
        ...
        save: function () {
            try {
                ...
                const tempId = this.$store.getters['shopCategory/temp'].temp_id;
                this.loading.isActive = true;
                this.$store.dispatch('shopCategory/save', {
                    form: fd,
                    search: this.props.search
                }).then((res) => {
                    appService.modalHide();
                    this.loading.isActive = false;
                    alertService.successFlip((tempId === null ? 0 : 1), this.$t('menu.shop_categories'));
                    this.props.form = {
                        name: "",
                        status: statusEnum.ACTIVE,
                        sort: null

                    ...
                    <div v-if="permissionChecker('items_create')" class="dropdown-group">
                        <ImportComponent />
                         <div class="dropdown-list db-card-filter-dropdown-list">
                            ...
                            <UploadBulkImageComponent :dataModal="'bulkImageUpload'"  @click="uploadModal('#bulkImageUpload')" />
                        </div>
                    </div>
                    <ItemUploadComponent v-on:list="list" :data-modal="'itemUpload'" :upload-type="'items'" />
                    <ItemUploadComponent v-on:list="list" :data-modal="'bulkImageUpload'" :upload-type="'images'" />
                ...
                <table class="db-table stripe" id="print" :dir="direction">
                    ...
                    <tbody class="db-table-body" v-if="items.length > 0">
                        <tr class="db-table-body-tr" v-for="item in items" :key="item.id">
                            <!-- to do  -->
                            <td class="db-table-body-td">{{ item.barcode != 'null' ? item.barcode : ""}}</td>
                            <td class="db-table-body-td">{{ item.item_code || "" }}</td>
                            <td class="db-table-body-td">
                                <img class="h-14 w-14 object-contain rounded-lg cursor-pointer" :src="item.thumb" alt="item"/>
                            </td>

                            <td class="db-table-body-td">{{ item.branch_id ? item.branch.name : ''}}</td>
                            <td class="db-table-body-td">{{ item.category_name }}</td>
                            <td class="db-table-body-td">
                                <!-- {{ textShortener(item.name, 40) }}
                                language_code: {{ language_code }} -->
                                {{ item['name_' + language_code] || item.name }}
                            </td>
                            ...
                            <td class="db-table-body-td">
                                <span :class="yesNoClass(item.manage_stock)">
                                    {{ enums.stockManagementEnumArray[item.manage_stock] }}
                                </span>
                            </td>
...
export default {
    ...
    methods: {
        ...
        yesNoClass: function (status) {
            return appService.yesNoClass(status);
        },
        ...
        clear: function () {
            ...
            this.props.search.kitchen_printer_id = null;
            this.props.search.label_printer_id = null;
            this.props.search.branch_id = null;
            this.props.search.barcode = "";
            this.props.search.stockManagement = null;
            this.list();
        },
        ...
        edit: function (item) {
            ...
            this.props.form = {
                ...
                status: item.status,
                kitchen_printer_id: item.kitchen_printer_id != null ? item.kitchen_printer_id.id : item.kitchen_printer_id,
                label_printer_id: item.label_printer_id != null ? item.label_printer_id : item.label_printer_id,
                branch_id: item.branch.id,
                barcode: item.barcode,
                stockManagement: item.manage_stock,
                is_print_menu: item.is_print_menu,
                is_print_label: item.is_print_label,
                tax_name: item.tax_name,
                tax_rate: parseFloat(item.tax_rate),
                tax_type: parseInt(item.tax_type),
                tax_amount: parseFloat(item.tax_amount),
                price_with_tax: parseFloat(item.price_with_tax),
                can_input_custom_name: item.can_input_custom_name,
                can_input_custom_unit_price: item.can_input_custom_unit_price,
                item_kind: item.item_kind ?? 1,

<template>
    ...
    <div class="col-12" v-if="Object.entries(employee).length > 0">
        ...
        <div class="flex flex-col items-start sm:flex-row sm:items-center gap-1.5 mb-6">
            ...
            <button v-if="therapistProfile" type="button" data-tab="#therapist"
                class="profile-tabBtn w-full justify-start sm:w-fit inline-flex items-center sm:justify-center gap-2 h-[38px] py-2 px-4 rounded-md text-[#6E7191] stroke-[#6E7191]">
                <i class="lab lab-user-line"></i>
                <span class="capitalize text-sm">{{ $t("menu.therapist_profiles") }}</span>
            </button>
        </div>
        ...
        <div id="therapist" class="profile-tabDiv" v-if="therapistProfile">
            <div class="db-card">
                <div class="db-card-header">
                    ...
                    <div class="row py-2">
                        <div class="col-12 sm:col-6 !py-1.5">
                            <div class="db-list-item p-0">
                                ...
                                <span class="db-list-item-text w-full sm:w-1/2">{{ therapistProfile.code || '-' }}</span>
                            </div>
                        </div>
                        <div class="col-12 sm:col-6 !py-1.5">
                            <div class="db-list-item p-0">
                                <span class="db-list-item-title w-full sm:w-1/2">{{ $t('label.commission_rate') }}</span>
                                <span class="db-list-item-text w-full sm:w-1/2">{{ therapistProfile.commission_rate }}%</span>
                            </div>
                        </div>
                        <div class="col-12 sm:col-6 !py-1.5">
                            <div class="db-list-item p-0">
                                <span class="db-list-item-title w-full sm:w-1/2">{{ $t('label.status') }}</span>
                                <span class="db-list-item-text w-full sm:w-1/2">{{ therapistProfile.status }}</span>
                            </div>
                        </div>
                        <div class="col-12 sm:col-6 !py-1.5" v-if="therapistProfile.verify_code">
                            <div class="db-list-item p-0">
                                <span class="db-list-item-title w-full sm:w-1/2">{{ $t('label.verify_code') }}</span>
                                <span class="db-list-item-text w-full sm:w-1/2">{{ therapistProfile.verify_code }}</span>
                            </div>
                        </div>
                        <div class="col-12 !py-3" v-if="therapistProfile.verify_code">
                            <p class="db-list-item-title mb-3">{{ $t('label.verify_code') }} QR</p>
                            <div v-if="qrCodeDataUrl" class="inline-block p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
                                <img :src="qrCodeDataUrl" alt="Verify Code QR" class="w-40 h-40" />
                            </div>
                            <p v-else class="text-sm text-gray-400">{{ $t('label.generating') }}...</p>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    mounted() {
        ...
        this.$store.dispatch('therapistProfile/lists', {
            ...
            if (list.length > 0) {
                this.therapistProfile = list[0];
                if (this.therapistProfile.verify_code) {
                    this.generateQrCode(this.therapistProfile.verify_code);
                }
            }
        }).catch(() => {});
        this.orderLists();
    ...
    methods: {
        ...
        generateQrCode: function (text) {
            QRCode.toDataURL(text, { width: 160, margin: 2 }).then((url) => {

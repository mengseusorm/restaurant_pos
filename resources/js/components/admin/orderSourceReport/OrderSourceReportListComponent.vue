    ...
    <div class="col-12">
        <div class="db-card db-tab-div active">
            <div class="db-card-header border-none">
                <h3 class="db-card-title">{{ $t('menu.order_source_report') }}</h3>
                <div class="db-card-filter">
                    ...
                    <PrintContentComponent modal-id="printOrderSourceReportModal" button-text="" :printers="invoicePrinters">
                        <template #body>
                            <div id="printOrderSourceReportModal-print-content" style="padding: 12px; font-size: 12px; max-width: 320px; margin: 0 auto;">
                                <!-- Header Section -->
                                ...
                                <div style="text-align: center; margin-bottom: 10px;">
                                    <h3 style="font-size: 12px; font-weight: bold; margin: 0; text-transform: uppercase;">{{ $t('menu.order_source_report') }}</h3>
                                </div>
                                ...
                                <table style="width: 100%; font-size: 9px;">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="text-align: left; padding: 3px 2px; font-weight: bold;">{{ $t('label.source') }}</th>
                                    ...
                                    <tbody v-if="orderSourceReports.length > 0">
                                        <tr v-for="(orderSourceReport, index) in orderSourceReports" :key="orderSourceReport">
                                            <td style="padding: 4px 2px;">{{ index + 1 }}</td>
                                            <td style="padding: 4px 2px;">{{ enums.sourceEnumArray[orderSourceReport.source] }}</td>
                                            <td style="text-align: center; padding: 4px 2px;">{{ orderSourceReport.total_orders }}</td>
                                            <td style="text-align: right; padding: 4px 2px;">{{ orderSourceReport.total_with_tax }}</td>
                                        </tr>
                                    </tbody>
                                </table>
            ...
            <div class="db-table-responsive">
                <table class="db-table stripe" id="print" :dir="direction">
                    ...
                    <tbody class="db-table-body" v-if="orderSourceReports.length > 0">
                        <tr class="db-table-body-tr" v-for="(orderSourceReport, index) in orderSourceReports" :key="orderSourceReport">
                            <td class="db-table-body-td">{{ index + 1 }}</td>
                            <td class="db-table-body-td">
                                <span class="db-table-badge text-green-600 bg-green-100">
                                    {{ enums.sourceEnumArray[orderSourceReport.source] }}
                                </span>
                            </td>
                            <td class="db-table-body-td">{{ orderSourceReport.total_orders }}</td>
                            <td class="db-table-body-td">{{ orderSourceReport.total }}</td>
                            <td class="db-table-body-td">{{ orderSourceReport.total_tax }}</td>
                            <td class="db-table-body-td">{{ orderSourceReport.total_with_tax }}</td>
                            <td class="db-table-body-td">{{ orderSourceReport.order_currency }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
export default {
    name: "OrderSourceReportListComponent",
    components: {
        TableLimitComponent,
        PaginationSMBox,
        PaginationBox,
    ...
    data() {
        return {
            ...
            printObj: {
                id: "print",
                popTitle: this.$t('menu.order_source_report')
            },
            props: {
                form: {
                    date: null,
    ...
    computed: {
        ...
        orderSourceReports: function () {
            return this.$store.getters['orderSourceReport/lists'];
        },
        items: function () {
        ...
        pagination: function () {
            return this.$store.getters['orderSourceReport/pagination'];
        },
        paginationPage: function () {
            return this.$store.getters['orderSourceReport/page'];
        },
        direction: function () {
        ...
        invoicePrinters: function () {
            return this.$store.getters['printer/lists']
                ...
                .map(printer => ({
                    ...printer,
                    printContentId: 'printOrderSourceReportModal-print-content'
                }));
        },
    },
    methods: {
        ...
        list: function (page = 1) {
            ...
            this.$store.dispatch('orderSourceReport/lists', this.props.search).then(res => {
                this.loading.isActive = false;
            }).catch((err) => {
                this.loading.isActive = false;
            });
        ...
        xls: function () {
            this.loading.isActive = true;
            this.$store.dispatch('orderSourceReport/export', this.props.search).then(res => {
                this.loading.isActive = false;
                const blob = new Blob([res.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
                const link = document.createElement('a');
                link.href = URL.createObjectURL(blob);
                link.download = this.$t("menu.order_source_report");
                link.click();
        ...
        xlsAll: function () {
            ...
            this.$store.dispatch('orderSourceReport/export', searchParams).then(res => {
                this.loading.isActive = false;
                const blob = new Blob([res.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
                const link = document.createElement('a');
                link.href = URL.createObjectURL(blob);
                link.download = this.$t("menu.order_source_report") + "_all.xlsx";
                link.click();
                URL.revokeObjectURL(link.href);
            }).catch((err) => {
                this.loading.isActive = false;
        ...
        pdf: function () {
            this.loading.isActive = true;
            this.$store.dispatch("orderSourceReport/pdf", this.props.search).then((res) => {
                this.loading.isActive = false;
                const blob = new Blob([res.data]);
                const link = document.createElement("a");
                link.href = URL.createObjectURL(blob);
                link.download = this.$t("menu.order_source_report") + ".pdf";
                link.click();
                URL.revokeObjectURL(link.href);
            }).catch((err) => {
                this.loading.isActive = false;
        ...
        totalAmountWithVat(items) {
            const total = items.reduce((acc, item) => acc + parseFloat(item.total_with_tax || 0), 0);

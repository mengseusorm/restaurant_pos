<template>
        ...
        <div class="db-card p-4">
            <div class="flex flex-wrap gap-y-5 items-end justify-between">
                <div>
                    ...
                    <ul class="flex flex-col gap-2">
                        <li class="flex items-center gap-2">
                            ...
                            <span class="text-sm">{{ order.order_datetime }}</span>
                        </li>
                        <li class="text-sm">
                        ...
                        <li class="flex items-start gap-2" v-if="order.payment_status == enums.paymentStatusEnum.PAID">
                            ...
                            <span class="text-heading flex-1"> 
                                <span :class="'ms-2 capitalize leading-5 px-3 rounded-xl text-[#FB4E4E] bg-[#FFDADA]' + statusClass(order.payment_status)">
                                    {{ enums.paymentStatusEnumArray[order.payment_status] }}
                                </span> 
                            </span>
                        </li>
                        ...
                        <li class="text-sm" v-if="order.table_name">
                            {{ $t("label.table_name") }}:
                            <span class="text-heading">
                                {{ order.table_name }}
                            </span>
                        </li>
                        <li class="text-sm" v-if="order.token">
                            {{ $t("label.token_no") }}:
                            <span class="text-heading">
                                #{{ order.token }}
                            </span>
                ...
                <div class="flex flex-wrap gap-3" v-if="order.status === enums.orderStatusEnum.PENDING">
                    <TelegramMiniAppOrderReasonComponent />
                ...
                <div class="flex flex-wrap gap-3" v-else-if="order.status !== enums.orderStatusEnum.REJECTED && order.status !== enums.orderStatusEnum.CANCELED">
                    <div class="relative" v-if="!order.token">
                        <TelegramMiniAppOrderTokenComponent />
                    </div>
                    ...
                    <TelegramMiniAppOrderReceiptComponent :order="order" :orderItems="orderItems" :orderUser="orderUser" />
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-12 sm:col-6">
        ...
        <div v-if="order?.order_note" class="row">
            <div class="col-12">
                <div class="db-card">
                    <div class="db-card">
                        <div class="db-card-header">
                            <h3 class="db-card-title">{{ $t("label.order_note") }}</h3>
                        </div>
                        <div class="db-card-body">
                            <p v-if="order?.order_note" style="white-space: pre-line;">
                                {{ order.order_note }}
                            </p>
                        </div>
                    </div>
        ...
        <div v-if="order && (order.customer_name || order.customer_phone_number || order.customer_address)" class="row">
            <div class="col-12">
                <div class="db-card">
                    <div class="db-card-header">
            ...
            <!-- <div class="col-12">
                <div class="db-card">
                    ...
                    <div class="db-card-body">
                        ...
                        <ul v-if="orderUser" class="flex flex-col gap-3 py-4 mb-4 border-y border-[#EFF0F6]">
...
import TelegramMiniAppOrderReasonComponent from "./TelegramMiniAppOrderReasonComponent";
import TelegramMiniAppOrderTokenComponent from "./TelegramMiniAppOrderTokenComponent";
import TelegramMiniAppOrderReceiptComponent from "./TelegramMiniAppOrderReceiptComponent";

export default {
    name: "TelegramMiniAppOrderShowComponent",
    components: {
        TelegramMiniAppOrderReceiptComponent,
        LoadingComponent,
        TelegramMiniAppOrderReasonComponent,
        TelegramMiniAppOrderTokenComponent
    },
    ...
    computed: {
        order: function () {
            return this.$store.getters["adminTelegramMiniAppOrder/show"];
        },
        orderItems: function () {
            return this.$store.getters["adminTelegramMiniAppOrder/orderItems"];
        },
        orderUser: function () {
            return this.$store.getters["adminTelegramMiniAppOrder/orderUser"];
        },
    },
    mounted() {
        this.loading.isActive = true;
        this.$store
            .dispatch("adminTelegramMiniAppOrder/show", this.$route.params.id)
            .then((res) => {
    ...
    methods: {
            ...
            appService
                ...
                .then((res) => {
                    try {
                        this.loading.isActive = true;
                        this.$store
                            .dispatch("adminTelegramMiniAppOrder/changeStatus", {
                        ...
                        if(status === this.enums.orderStatusEnum.ACCEPT) {
        ...
        changePaymentStatus: function (e) {
            try {
                this.loading.isActive = true;
                this.$store
                    .dispatch("adminTelegramMiniAppOrder/changePaymentStatus", {
                        id: this.$route.params.id,

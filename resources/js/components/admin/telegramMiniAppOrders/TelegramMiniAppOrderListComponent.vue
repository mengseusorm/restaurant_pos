    ...
    <div class="col-12">
        <div class="db-card">
            <div class="db-card-header border-none">
                <h3 class="db-card-title">{{ $t("menu.telegram_mini_app") }}</h3>
                ...
                <table class="db-table stripe" id="print" :dir="direction">
                    <thead class="db-table-head">
                        <tr class="db-table-head-tr">
                            ...
                            <th class="db-table-head-th hidden-print" v-if="permissionChecker('telegram-mini-app')">
                                {{ $t("label.action") }}
                            </th>
                        </tr>
                    </thead>
                    <tbody class="db-table-body" v-if="orders.length > 0">
                        <tr class="db-table-body-tr cursor-pointer"
                            ...
                            :key="order"
                            @click="$router.push({ name: 'admin.telegram.mini.app.order.show', params: { id: order.id } })" >
                            ...
                            <td class="db-table-body-td hidden-print" v-if="permissionChecker('telegram-mini-app')">
                                <div class="flex justify-start items-center sm:items-start sm:justify-start gap-1.5">
                                    <SmIconViewComponent :link="'admin.telegram.mini.app.order.show'" :id="order.id"
                                        v-if="permissionChecker('telegram-mini-app')" />
                                </div>
                            </td>
                        </tr>
                    </tbody>
...
export default {
    ...
    data() {
        return {
            ...
            printObj: {
                id: "print",
                popTitle: this.$t("menu.telegram_mini_app"),
            },
            props: {
                form: {
                    date: new Date(),
    ...
    computed: {
        orders: function () {
            return this.$store.getters["adminTelegramMiniAppOrder/lists"];
        },
        customers: function () {
            return this.$store.getters["user/lists"];
        },
        pagination: function () {
            return this.$store.getters["adminTelegramMiniAppOrder/pagination"];
        },
        paginationPage: function () {
            return this.$store.getters["adminTelegramMiniAppOrder/page"];
        },
        direction: function () {
    ...
    methods: {
        ...
        list: function (page = 1) {
            ...
            this.$store
                .dispatch("adminTelegramMiniAppOrder/lists", this.props.search)
                .then((res) => {
        ...
        xls: function () {
            this.loading.isActive = true;
            this.$store
                .dispatch("adminTelegramMiniAppOrder/export", this.props.search)
                .then((res) => {
                    this.loading.isActive = false;
                    ...
                    link.download = this.$t("menu.telegram_mini_app");
                    link.click();

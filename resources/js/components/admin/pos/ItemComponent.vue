<template>
    <div class="grid gap-3 sm:gap-[18px] grid-cols-[repeat(auto-fill,_minmax(140px,_1fr))] sm:grid-cols-[repeat(auto-fill,_minmax(185px,_1fr))] mb-8 md:mb-0">
        <div v-for="item in items" :key="item"
            class="rounded-2xl border transition border-[#EFF0F6] bg-white hover:shadow-lg cursor-pointer overflow-hidden">
            <div class="relative">
                <img class="w-full h-40 sm:h-44 object-cover" :src="item.thumb" alt="item" @click="order(item)">
            </div>
            <div class="p-3 sm:p-4">
                <h3 class="text-xs sm:text-base font-medium font-rubik capitalize text-gray-900 mb-2 line-clamp-1 leading-tight">
                    {{ textShortener(item.name, 25) }}
                </h3>
                <div class="mb-3">
                    <span v-if="item.barcode && item.barcode != 'null'" class="text-xs font-rubik text-gray-500 flex items-center">
                ...
                <div class="flex items-center justify-between">
                    <div class="price">
                        <h4 v-if="getItemOffer(item)" class="font-rubik font-semibold text-gray-900">
                            <span class="line-through text-gray-400 text-sm mr-2">{{ item.branch_currency_price }}</span>
                            <span class="text-primary">{{ getItemOffer(item)?.currency_price }}</span>
                        </h4>
                        <h4 v-else class="font-rubik font-semibold text-gray-900">
                            {{ item.branch_currency_price }}
                        </h4>
                    </div>
                    <button @click.prevent="variationModalShow(item)" data-modal="#item-variation-modal"
                            class="flex items-center justify-center w-8 h-8 rounded-full bg-primary text-white hover:bg-primary/90 hover:shadow-md transition-all duration-200">
            ...
            <div class="modal-header items-start border-none pb-0">
                <div class="flex gap-4">
                    ...
                    <div class="flex-auto">
                        ...
                        <h4 class="text-sm font-semibold">{{ item.offer.length > 0 ? item.offer[0].branch_currency_price : item.branch_currency_price }}</h4>
...
export default {
    ...
    methods: {
        ...
        variationModalShow: function (item) {
            this.item = item;
            const offer = this.getItemOffer(item);
            ...
            this.temp.convert_price = offer ? offer.convert_price : item.convert_price;
            this.temp.currency_price = offer ? offer.currency_price : item.currency_price;
            this.temp.total_price = (offer ? offer.convert_price : item.convert_price) + this.temp.item_variation_total;
            this.temp.can_input_custom_name = this.item.can_input_custom_name || null;
            this.temp.order_item_custom_name = null;
            ...
            this.itemArrays = [
                {
                    ...
                    can_input_custom_name: this.temp.can_input_custom_name,
                    order_item_custom_name: this.temp.order_item_custom_name
                }
            ];
        ...
        order: async function (item) {
            await this.quickAddToCart(item);
        },
        getItemOffer(item) {
            return item?.offer?.length > 0 ? item.offer[0] : null;
        },
    },

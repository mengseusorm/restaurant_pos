<template>
    <section class="mb-16 mt-4">
        <div class="container">
            <OnlineOrderLoadingComponent :props="loading" />
            ...
            <div class="swiper mb-7 menu-swiper" v-if="categories.length > 1">
                <Swiper :speed="1000" slidesPerView="auto" :spaceBetween="16" class="menu-slides" dir="rtl">
                    <SwiperSlide class="!w-fit relative" v-for="(category, index) in categories" :key="category" :class="category.id === itemProps.search.item_category_id || (category.id === 0 && itemProps.search.item_category_id === '') ? 'pos-group' : ''">
                        <router-link v-if="index === 0" to="" @click.prevent="allCategory(category)" class="w-32 flex flex-col items-center text-center gap-4 p-3 rounded-2xl border-b-2 border-transparent transition hover:bg-primary-light bg-[#F7F7FC] overflow-hidden">
                            <img class="h-10 drop-shadow-category" :src="category.thumb" alt="category" />
                            <h3 class="w-full text-xs leading-[16px] whitespace-nowrap overflow-hidden text-ellipsis font-medium font-rubik">
                                {{ category.name }}
                            </h3>
                        </router-link>
                        <router-link v-else to="" @click.prevent="setCategory(category.id, category.slug)" class="w-32 flex flex-col items-center text-center gap-4 p-3 rounded-2xl border-b-2 border-transparent transition hover:bg-primary-light bg-[#F7F7FC] overflow-hidden">
                            <img class="h-10 drop-shadow-category" :src="category.thumb" alt="category" />
                            <h3 class="w-full text-xs leading-[16px] whitespace-nowrap overflow-hidden text-ellipsis font-medium font-rubik">
                                {{ category.name }}
                            </h3>
            ...
            <!-- <div v-if="categories.length > 0" class="flex flex-wrap gap-3 w-full mb-5 veg-navs">
                <button
                    ...
                    :class="itemProps.property.type === enums.itemTypeEnum.NON_VEG ? 'veg-active' : ''"
                    type="button"
                ...
                >
                    ...
                    <i class="lab-close-circle-line text-xl text-red-500 transition opacity-0 ltr:-ml-8 rtl:-mr-8 clear-item-type-filter font-fill-danger lab-font-size-24"></i>
                </button>
                <button
                    ...
                    :class="itemProps.property.type === enums.itemTypeEnum.VEG ? 'veg-active' : ''"
                    type="button"
                    class="flex items-center gap-3 w-fit pl-3 pr-4 py-1.5 rounded-3xl transition hover:shadow-filter hover:bg-white bg-[#EFF0F6]"
                >
                    ...
                    <i class="lab-close-circle-line text-xl text-red-500 transition opacity-0 ltr:-ml-8 rtl:-mr-8 font-fill-danger lab-font-size-24"></i>
                </button>
            </div> -->
            ...
            <div v-if="Object.keys(category).length > 0" class="flex gap-4 items-center justify-between mb-6">
                ...
                <div class="flex items-center gap-3">
                    <button type="button" class="lab lab-row-vertical lab-font-size-20 text-xl" v-on:click="itemProps.property.design = enums.itemDesignEnum.LIST" :class="itemProps.property.design === enums.itemDesignEnum.LIST ? 'text-heading' : 'text-[#A0A3BD]'"></button>
                    <button type="button" class="lab lab-element-3 lab-font-size-20 text-xl" v-on:click="itemProps.property.design = enums.itemDesignEnum.GRID" :class="itemProps.property.design === enums.itemDesignEnum.GRID ? 'text-heading' : 'text-[#A0A3BD]'"></button>
                </div>
            </div>

            <OnlineOrderItemComponent :items="items" :type="itemProps.property.type" :design="itemProps.property.design" />
        </div>
    </section>
        ...
        <div class="modal-dialog max-w-[360px] relative">
            ...
            <div class="modal-body">
                ...
                <div class="flex gap-6" v-if="setting.site_online_payment_gateway === enums.activityEnum.ENABLE && order.transaction === null && order.payment_status === enums.paymentStatusEnum.UNPAID && paymentMethod === 'digitalPayment'">
                    <router-link @click.prevent="closeModal" class="w-full rounded-3xl text-center font-medium leading-6 py-3 border border-primary text-primary bg-white" :to="{ name: 'online.order.order.details', params: { slug: this.$route.params.slug, id: order.id } }">
                ...
                <router-link v-else @click.prevent="closeModal" class="w-full rounded-3xl text-center font-medium leading-6 py-3 text-white bg-primary" :to="{ name: 'online.order.order.details', params: { slug: this.$route.params.slug, id: order.id } }">
                    {{ $t('button.go_to_order') }}
                </router-link>
            </div>
        </div>
...
<script>
import OnlineOrderLoadingComponent from '../../onlineOrder/components/OnlineOrderLoadingComponent.vue';
import statusEnum from '../../../enums/modules/statusEnum';
import OnlineOrderItemComponent from '../components/OnlineOrderItemComponent.vue';
...
export default {
    name: 'OnlineOrderTableMenuComponent',
    components: {
        OnlineOrderItemComponent,
        OnlineOrderLoadingComponent,
        Swiper,
        SwiperSlide,
    },
    // props: {
    ...
    //         immediate: true,
    //         handler(newSlug) {
    //             this.itemProps.search.item_category_id = newSlug;
    //             this.itemList();
    //         }
    //     }
    // },
    ...
    computed: {
        ...
        order: function () {
            return this.$store.getters['onlineOrderOrder/show'];
        },
        paymentMethod: function () {
            return this.$store.getters['onlineOrderCart/paymentMethod'];
        },
    },
    async mounted() {
        // console.log('Online Order Menu Component Mounted');
        // alert("Mounted Online Order Menu Component");

        this.loading.isActive = true;

        await this.$store
            .dispatch('onlineOrderBranch/show', this.$route.params.slug) //TODO:
            .then((res) => {
                // this.$store.dispatch('tableCart/initTable', res.data.data);

                this.branch_id = res.data.data.branch_id;

                // console.log('res branch: ', res);

                this.itemProps.search.branch_id = res.data.data.branch_id;
                this.categoryProps.search.branch_id = res.data.data.branch_id;
            })
            .catch((err) => {
    ...
    methods: {
        ...
        setCategory: function (id, slug = null) {
            ...
            if (slug !== null) {
                this.loading.isActive = true;
                this.$store
                    .dispatch('onlineOrderItemCategory/show', {
                        slug: slug,
                    })
                    .then((res) => {
        ...
        categoryList: function () {
            this.$store
                .dispatch('onlineOrderItemCategory/lists', this.categoryProps.search)
                .then((res) => {
                    this.loading.isActive = false;
        ...
        showOrderModal: function () {
            if (Object.keys(this.$route.query).length > 0) {
                this.loading.isActive = true;
                this.$store
                    .dispatch('onlineOrderOrder/show', this.$route.query.id) //TODO:
                    .then((res) => {
                        const modalTarget = this.$refs.confirmOrder;

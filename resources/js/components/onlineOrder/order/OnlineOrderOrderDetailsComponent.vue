                ...
                <div class="w-full">
                    <div class="p-4 mb-6 rounded-2xl shadow-xs bg-white">
                        ...
                        <div class="mt-4" v-if="parseInt(order.status) === parseInt(enums.orderStatusEnum.REJECTED)">
                            ...
                            <p class="text-sm text-heading mb-2">{{ order.rejection_reason }}</p>
                            <p>
                                <span class="text-sm text-heading">{{ $t("label.rejected_at") }}: {{ order.rejected_at }}</span>
                            </p>
                            <p class="text-sm text-orange-600 bg-orange-50 p-2 rounded border-l-4 border-orange-400">
                                <i class="lab lab-info-circle mr-1"></i>
                                {{ $t("label.note") }}: {{ $t("label.if_paid_check_refund_amount") }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div v-if="parseInt(order.status) !== parseInt(enums.orderStatusEnum.REJECTED) && parseInt(order.status) !== parseInt(enums.orderStatusEnum.CANCELED)" class="p-4 rounded-2xl shadow-xs bg-white">
                        ...
                        <ul class="flex flex-col gap-2 mb-2">
                            <!-- <li v-if="enums.paymentStatusEnum.PAID === order.payment_status" class="flex items-center gap-2"> -->
                            <li class="flex items-center gap-2">
                                <span class="capitalize text-sm leading-6">{{ $t("label.method") }}:</span>
                                <span v-if="order" class="capitalize text-sm leading-6 text-heading">
...
export default {
    name : "OnlineOrderOrderDetailsComponent",
    components: {OrderReceiptComponent, OrderStatusComponent, OnlineOrderLoadingComponent},
    data() {
        return {
            loading: {
                isActive: false,
    ...
    mounted() {
        ...
        if (this.$route.params.id) {
            this.loading.isActive = true;
            this.$store.dispatch("onlineOrderOrder/show", this.$route.params.id).then(res => {
                this.$store.dispatch("onlineOrderCart/resetPaymentMethod").then().catch();

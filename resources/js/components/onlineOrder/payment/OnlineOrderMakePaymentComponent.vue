            ...
            <div class="flex items-start flex-col md:flex-row gap-6">
                <div class="w-full">
                    <div class="p-4 mb-6 rounded-2xl shadow-xs bg-white">
                        ...
                        <!-- <OnlineOrderOrderStatusComponent :props="order" /> -->
                ...
                <div class="w-full rounded-2xl shadow-xs bg-white">
                    ...
                    <!-- <div class="p-4">
                        <OnlineOrderOrderReceiptComponent :order="order" :orderBranch="orderBranch" :orderItems="orderItems" />
                    </div> -->
                </div>
            </div>
        </div>
...
import OnlineOrderOrderStatusComponent from '../../onlineOrder/components/OnlineOrderOrderStatusComponent.vue';
import OnlineOrderOrderReceiptComponent from '../../onlineOrder/order/OnlineOrderOrderReceiptComponent.vue';
...
export default {
    name: 'OnlineOrderMakePaymentComponent',
    components: { OnlineOrderOrderReceiptComponent, OnlineOrderOrderStatusComponent, OnlineOrderLoadingComponent },
    data() {
        return {
            loading: {
                isActive: false,
    ...
    computed: {
        ...
        orderBranch: function () {
            return this.$store.getters['onlineOrderOrder/orderBranch'];
        },
        orderItems: function () {
            return this.$store.getters['onlineOrderOrder/orderItems'];
        },
    ...
    mounted() {
        ...
        if (this.$route.params.id) {
            this.loading.isActive = true;
            this.$store
                .dispatch('onlineOrderOrder/show', this.$route.params.id)
                .then((res) => {
                    ...
                    if (res.data.data.payment_status == this.enums.paymentStatusEnum.PAID) {
                        this.$router.push({ name: 'online.order.order.details', params: { id: this.$route.params.id } });
                    }
                })
                .catch((error) => {
                    this.loading.isActive = false;
